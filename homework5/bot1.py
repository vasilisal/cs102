...
def get_schedule(web_page: str, day: str):
    ...
    date = str(days.index(day) + 1) + "day" # +1 так как отсчет начинается не с понедельника

    # Получаем таблицу с расписанием
    schedule_table = soup.find("table", attrs={"id": date}) #find и find_all позволяют найти теги с указанными атрибутами
    if schedule_table is None:
        return "error"

    # Время проведения занятий
    times_lst = schedule_table.find_all("td", attrs={"class": "time"}) #возвращает список всех найденных совпадений
